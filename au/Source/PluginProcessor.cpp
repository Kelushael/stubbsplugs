...
std::vector<NeonForgeProcessor::DerivedParam> NeonForgeProcessor::getDerivedParams() const
{
    ...
    for (int i = 0; i < profile.eqBands.size(); ++i)
    {
        ...
        out.emplace_back (juce::String ("EQ") + juce::String (i+1) + " Freq", &b.freq, 20.0f, 20000.0f);
        out.emplace_back (juce::String ("EQ") + juce::String (i+1) + " Gain", &b.gain, -18.0f, 18.0f);
        out.emplace_back (juce::String ("EQ") + juce::String (i+1) + " Q", &b.q, 0.1f, 10.0f);
    }
    if (profile.comp.enabled)
    {
        out.emplace_back (juce::String ("Threshold"), &profile.comp.threshold_db, -60.0f, 0.0f);
        out.emplace_back (juce::String ("Ratio"), &profile.comp.ratio, 1.0f, 20.0f);
        out.emplace_back (juce::String ("Attack"), &profile.comp.attack_ms, 0.1f, 200.0f);
        out.emplace_back (juce::String ("Release"), &profile.comp.release_ms, 1.0f, 2000.0f);
    }
    if (profile.sat.enabled)
    {
        out.emplace_back (juce::String ("Drive"), &profile.sat.drive_db, 0.0f, 24.0f);
        out.emplace_back (juce::String ("Sat Mix"), &profile.sat.mix, 0.0f, 1.0f);
    }
    return out;
}
